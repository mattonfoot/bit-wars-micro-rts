  ...
  renderCodex() {
    ...
    if (cur.kind === 'unit') {
      ...
      stats = stat('Cost', `${d.cost.ore} ore${d.cost.flux ? ` · ${d.cost.flux} flux` : ''}`) + stat('Population', d.pop) + stat('Squad', d.size > 1 ? `${d.size} members` : d.hero ? 'Hero (one only)' : 'Single') +
        ...
        stat('Splash', w.splash ? `${w.splash} tiles` : undefined) + stat('Vs structures', w.structure ? `×${w.structure} incendiary` : undefined) + stat('Set-up', w.setup ? `${w.setup}s` : undefined) + stat('Flies', d.flying ? 'Yes' : undefined) + stat('Captures', d.canCapture ? `Yes (×${d.capRate})` : 'No') +
