...
for (const camp of Object.values(CAMPAIGNS)) {
  for (const ch of camp.chapters) {
    ...
    const w = c.world;
    const ai = ch.enemy.ai ? new AI(w, 1, ch.enemy.ai === 'passive' ? 'normal' : ch.enemy.ai, { passive: ch.enemy.ai === 'passive' }) : null;
    ...
    for (let i = 0; i < 60 * 45; i++) {
      w.tick(TICK); ai?.update(TICK); c.onEvents(w.events); c.update(TICK); w.events.length = 0; w.dirtyTiles.length = 0;
    }
    assert.equal(c.status, 'playing', `${ch.key} should still be playing after 45s (was ${c.status}: ${c.reason})`);
    ...
    n++;
    console.log(`${ch.key.padEnd(8)} ${ch.title.padEnd(20)} ok · squads ${w.squads.length} · buildings ${w.buildings.length} · stage ${c.stage} · objectives ${c.list().map((o) => `${o.cur}/${o.target}`).join(' ')}`);
  }
}
